...
export const ScreenChatTab: React.FC<ScreenChatTabProps> = ({ onScreenCapture }) => {
  ...
  const captureScreenshot = async () => {
    try {
      ...
      setScreenImage(screenshot);
      
      // Add confirmation message
      const newMessage: ChatMessage = {
        role: "assistant",
        content: "Screenshot captured! You can now ask me questions about what's visible on your screen.",
        timestamp: new Date().toLocaleTimeString()
      };
      setChatMessages([newMessage]);
      
      if (onScreenCapture) {
        onScreenCapture(screenshot);
      }
      ...
      const errorMessage: ChatMessage = {
        role: "assistant",
        content: "Failed to capture screenshot. Please ensure you have granted the necessary permissions.",
        timestamp: new Date().toLocaleTimeString()
      };
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage: ChatMessage = {
        role: "assistant",
        content: "Sorry, I couldn't process your request. Please check your API key configuration.",
        timestamp: new Date().toLocaleTimeString()
      };
  ...
  const sendToVisionAI = async (message: string, base64Image: string): Promise<string> => {
  ...
  const renderMessages = () => {
    if (!isApiKeySet) {
      return (
        <div style={{
          ...
          <p style={{ fontSize: '0.85rem', color: '#94a3b8', marginBottom: '1rem', maxWidth: '280px', lineHeight: '1.5' }}>
            Please configure your AI API key to use screen chat functionality.
          </p>
          <button
            onClick={() => window.open(chrome.runtime.getURL('/apikey.html'), '_blank')}
            style={{
      ...
      return (
        <div style={{
          ...
          <p style={{ fontSize: '0.85rem', color: '#94a3b8', marginBottom: '1.5rem', maxWidth: '280px', lineHeight: '1.5' }}>
            Take a screenshot of your current page and chat with AI about what's visible. Get visual insights and ask questions about the interface.
          </p>
          <button
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        {/* Chat Messages */}
        <div style={{ flex: 1, overflowY: 'auto', marginBottom: '1rem', padding: '0.5rem 0' }}>
        ...
        <form onSubmit={handleSendMessage} style={{ display: 'flex', gap: '0.5rem' }}>
          ...
          <button
            ...
            style={{
              padding: '0.75rem 1rem',
              background: isProcessing || !inputText.trim() || !screenImage
                ? 'rgba(34, 197, 94, 0.3)'
                : 'linear-gradient(135deg, rgba(34, 197, 94, 0.8), rgba(132, 204, 22, 0.8))',
              border: '1px solid rgba(34, 197, 94, 0.5)',
              borderRadius: '0.5rem',
              color: 'white',
              cursor: isProcessing || !inputText.trim() || !screenImage ? 'not-allowed' : 'pointer',
              transition: 'all 0.2s ease'
            }}
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
